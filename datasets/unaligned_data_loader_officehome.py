...
from datasets_officehome import Dataset
import random
...
class PairedData(object):
    ...
    def __next__(self):
        ...
        if (self.stop_A and self.stop_B and self.stop_C and self.stop_t) or self.iter > self.max_dataset_size:
            self.stop_A = False
            self.stop_B = False
            self.stop_C = False
            self.stop_t = False
...
class UnalignedDataLoaderofficehome():
    def initialize(self, source, target, batch_size1, batch_size2, scale=256):
        ...
        self.paired_data = PairedData(data_loader_s1, data_loader_s2, data_loader_s3, data_loader_t,
                                      float("inf"))
       

    def name(self):
        return 'UnalignedDataLoaderofficehome'
